...
const servicesItems = [
  ...
  { name: 'Marine Survey and Consultancy', path: '/marinescservices' },
...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full bg-white text-gray-800 shadow-md z-50 transition-all duration-300">
      ...
      >
        ...
        <div className="hidden md:flex items-center space-x-4">
          ...
          <a
            ...
            className="bg-gray-300 hover:bg-blue-500 text-gray-800 px-2 py-1 rounded-md font-semibold transition duration-200 transform hover:scale-105"
          >
            License and Info
          </a>
          <a
      ...
      {/* Desktop Navigation */}
      <div
        className={`hidden md:flex justify-center bg-blue-500 transition-all duration-500
      ...
      >
        <ul className="flex space-x-8 py-4 items-center text-lg font-medium text-white">
          ...
          <li>
            <Link to="/aboutus" className="px-4 py-2 hover:bg-blue-700 rounded-md transition duration-300" onClick={closeAll}>
              About Us
            </Link>
          </li>
          {/* Services Dropdown */}
          ...
          >
            <div className="flex items-center px-4 py-2 hover:bg-blue-700 rounded-md transition duration-300 select-none">
              Services
              <span className="ml-2">
            ...
            <div
              className={`absolute ${openDropdown === 'services' ? 'flex' : 'hidden'} flex-col bg-blue-700 text-white shadow-lg rounded-md mt-2 min-w-[200px] z-10 transition-all ease-in-out duration-300`}
            >
              {servicesItems.map((item, idx) => (
                <Link
                  key={idx}
                  to={item.path}
                  className="px-3 py-1 hover:bg-blue-600 transition duration-300"
                  onClick={closeAll}
                >
                  {item.name}
                </Link>
          ...
          <li>
            <Link to="/qualitypolicy" className="px-4 py-2 hover:bg-blue-700 rounded-md transition duration-300" onClick={closeAll}>
              Quality Policy
            </Link>
          </li>
          <li>
            <Link to="/codeofethics" className="px-4 py-2 hover:bg-blue-700 rounded-md transition duration-300" onClick={closeAll}>
              Code of Ethics
            </Link>
          </li>
          {/* Vacancies Dropdown */}
          ...
          >
            ...
            <div
              className={`absolute ${openDropdown === 'vacancies' ? 'flex' : 'hidden'} flex-col bg-blue-700 text-white shadow-lg rounded-md mt-2 min-w-[200px] z-10 transition-all ease-in-out duration-300`}
            >
              {vacanciesItems.map((item, idx) => (
                <Link
                  key={idx}
                  to={item.path}
                  className="px-4 py-2 hover:bg-blue-600 transition duration-300"
                  onClick={closeAll}
                >
                  {item.name}
                </Link>
          ...
          <li>
            <Link to="/contactus" className="px-4 py-2 hover:bg-blue-700 rounded-md transition duration-300" onClick={closeAll}>
              Contact Us
            </Link>
          </li>
          {/* Get a Quote Button */}
